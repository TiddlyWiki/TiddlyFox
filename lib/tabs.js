...
"use strict";
 ...
 var SAVING_PROMPT_MESSAGE = [
 	"<h1>This TiddlyWiki file can be used with TiddlyWiki for Firefox</h1>",
 	"<p>Click the toolbar button to enable saving.</p>",
 	"<p><img width='64px' src='data:image/svg+xml,",
 	encodeURIComponent(require("sdk/self").data.load("./images/tiddlyfox-alert.svg")),
 	"'></p>",
 	"<p>See <a href='http://tiddlywiki.com/#Saving%20with%20TiddlyFox' target='_blank' rel='noopener noreferrer'>the documentation</a> or <a href='http://groups.google.com/group/TiddlyWiki' target='_blank' rel='noopener noreferrer'>the discussion forum</a>.<p/>",
 	"<p><small>(click this message to close)</small></p>"
].join("\n");
		...
		if(message.isLocalFile && message.isTiddlyWiki) {
			...
			if(self.getTiddlyTabByUrl(urlString,theID)) {
				...
				if(self.tiddlyPermissions.getUrlPermission(urlString) === "all") {
					...
					tiddlyTab.worker.port.emit("prompt-saving",{html: SAVING_PROMPT_MESSAGE});
					tiddlyTab.state = "ask";
				}
			}
		} else {
